...
def _highlight_stroke(painter, pts, color, width, clip_rect):
    """Semi-transparent highlighter — keeps text readable underneath."""
    if len(pts) < 2:
        return
    painter.save()
    painter.setBrush(Qt.BrushStyle.NoBrush)
    ...
    hi_color.setAlphaF(0.38)
    hi_width = max(width * 2.0, 10.0)
    painter.setPen(QPen(hi_color, hi_width,
                        Qt.PenStyle.SolidLine,
                        Qt.PenCapStyle.RoundCap,
                        Qt.PenJoinStyle.RoundJoin))
    p1 = p2 = pts[0]; p3 = pts[1]
...
class DrawZone(QWidget):
    ...
    def __init__(self, parent=None):
        ...
        # Pen type: "normal" or "highlight"
        self._pen_type = "normal"
    ...
    def paintEvent(self, event):
        p = QPainter(self)
        p.setRenderHint(QPainter.RenderHint.Antialiasing)
        # ── Corner handles drawn directly to the widget ───────────────────
        r   = self.rect()
        ...
        # ── Ink layer: render to an off-screen QPixmap first ─────────────
        # CompositionMode_Clear does NOT work reliably when painting directly
        # to a widget surface — it clears to opaque black on most platforms
        # instead of transparent. Painting to a QPixmap with an alpha channel
        # first makes CompositionMode_Clear behave correctly (clears to
        # transparent), then we blit the finished pixmap onto the widget.
        inner  = self._draw_area()
        ink_px = QPixmap(self.size())
        ink_px.fill(Qt.GlobalColor.transparent)
        ip = QPainter(ink_px)
        ip.setRenderHint(QPainter.RenderHint.Antialiasing)
        ip.setClipRect(inner)
        for s in self._strokes:
            if len(s['pts']) < 2: continue
            if s['erasing']:
                _eraser_stroke(ip, s['pts'], s['width'], inner)
            elif s.get('pen_type', 'normal') == 'highlight':
                _highlight_stroke(ip, s['pts'], s['color'], s['width'], inner)
            else:
                _smooth_stroke(ip, s['pts'], s['color'], s['width'], inner)
        if self._cur_pts and len(self._cur_pts) >= 2:
            if self._cur_erasing:
                _eraser_stroke(ip, self._cur_pts, self._cur_width, inner)
            elif getattr(self, '_cur_pen_type', 'normal') == 'highlight':
                _highlight_stroke(ip, self._cur_pts, self._cur_color, self._cur_width, inner)
            else:
                _smooth_stroke(ip, self._cur_pts, self._cur_color, self._cur_width, inner)
        ip.end()
        p.drawPixmap(0, 0, ink_px)
    ...
    def _begin_stroke(self, pos):
        ...
        self._cur_pen_type = "normal" if erase else getattr(self, "_pen_type", "normal")
        ...
        if self._drawing and len(self._cur_pts) >= 2:
            if self._cur_erasing and self._erase_mode == "line":
                ...
                self._strokes.append({'pts': pts, 'color': QColor(self._cur_color),
                                       ...
                                       'pen_type': getattr(self, '_cur_pen_type', 'normal')})
...
class DrawToolbar(QWidget):
    ...
    def __init__(self, zone, parent=None):
        ...
        # Pen type: "normal" / "highlight"
        self._pen_type = cfg.get("pen_type", "normal")
    ...
    WRITING_MODES = [
        ("fast",      "Quick",   "Lowest latency"),
        ("balanced",  "Study",   "Balanced — default"),
        ("precision", "Precise", "Maximum smoothing"),
    ]
    PEN_TYPES = [
        ("normal",    "Normal",    "Regular pen"),
        ("highlight", "Highlight", "Semi-transparent marker"),
    ]

    def _show_pen_menu(self):
        """Hold pen icon → two-section menu: Pen Type + Writing Mode."""
        dark = _is_dark()
        bg   = "#2c2c2c" if dark else "#ffffff"
        txt  = "#eeeeee" if dark else "#222222"
        sel  = "#3a3a3a" if dark else "#f0f0f0"
        bdr  = "#444"    if dark else "#e0e0e0"
        dim  = "#777"    if dark else "#aaa"
        menu = QMenu(self)
        menu.setStyleSheet(f"""
            ...
            QMenu::item:disabled {{ color: {dim}; padding: 2px 8px;
                                    font-size: 10px; letter-spacing: 0.5px; }}
            QMenu::separator {{ height: 1px; background: {bdr}; margin: 3px 6px; }}
        """)
        # ── Section A: Pen Type ───────────────────────────────────────────
        hdr_type = menu.addAction("PEN TYPE")
        hdr_type.setEnabled(False)
        type_acts = {}
        for val, label, desc in self.PEN_TYPES:
            a = menu.addAction(f"  {label}  —  {desc}")
            a.setCheckable(True)
            a.setChecked(self._pen_type == val)
            type_acts[a] = val
        menu.addSeparator()
        # ── Section B: Writing Mode ───────────────────────────────────────
        hdr_mode = menu.addAction("WRITING MODE")
        hdr_mode.setEnabled(False)
        mode_acts = {}
        ...
        def pick(action):
            if action in type_acts:
                self._set_pen_type(type_acts[action])
            elif action in mode_acts:
                self._set_writing_mode(mode_acts[action])
    ...
    def _set_pen_type(self, pen_type):
        ...
        self.zone._pen_type = pen_type
        try: c = _cfg(); c["pen_type"] = pen_type; _save_cfg(c)
        except Exception: pass

    def _set_writing_mode(self, mode):
        self._writing_mode = mode
        self.zone._writing_mode = mode
    ...
    def _on_pen_click(self):
        """
        Called by _HoldButton.short_action (user left-clicked the pen button).
        The button state has NOT been toggled yet — we toggle it here and apply.
        When called from _toggle_pen() the state IS already set — use _apply_pen_state().
        """
        self.pen_btn.setChecked(not self.pen_btn.isChecked())
        self._apply_pen_state()

    def _apply_pen_state(self):
        """Apply the current pen_btn.isChecked() state to the zone and toolbar."""
        chk = self.pen_btn.isChecked()
        print(f"[MDP] Pen state → {'ACTIVE' if chk else 'INACTIVE'}")
        self._expanded = chk
        self.zone.set_pen()
...
def _auto_activate():
    ...
    if tb and not tb.pen_btn.isChecked():
        tb.pen_btn.setChecked(True); tb._apply_pen_state()
...
def _deactivate_pen():
    ...
    if tb and tb.pen_btn.isChecked():
        tb.pen_btn.setChecked(False); tb._apply_pen_state()

def _toggle_pen():
    print("[MDP] _toggle_pen() called")
    tb = _controller._toolbar
    if tb is None:
        print("[MDP] _toggle_pen: toolbar is None — aborting")
        return
    tb.pen_btn.setChecked(not tb.pen_btn.isChecked())
    tb._apply_pen_state()
...
def _pen_key_label(key):
    mouse_map = {"mouse_middle": "Middle mouse button",
    ...
    if key.startswith("Qt:"):
        try:
            qt_key = Qt.Key(int(key[3:]))
            # QKeySequence gives us a human-readable name
            ks = QKeySequence(int(qt_key)).toString()
            return f"Key: {ks}" if ks else f"Key: {key}"
        except Exception:
            return f"Key: {key}"
    return f"Key: {key}"
...
class _KeyFilter(QObject):
    def eventFilter(self, obj, event):
        ...
        if event.type() == QEvent.Type.MouseButtonPress:
            btn = event.button()
            # Middle click only toggles when it is the configured shortcut key
            if btn == Qt.MouseButton.MiddleButton and key == "mouse_middle":
        ...
        if event.type() == QEvent.Type.KeyPress:
            ...
            if not key.startswith("mouse_"):
                # QShortcut is the primary handler; filter is backup only.
                # Skip if the shortcut is already active to avoid double-toggle.
                sc = _pen_shortcut[0]
                if sc is not None and sc.isEnabled():
                    pass   # QShortcut handles it
                else:
                    ek   = event.key()
                    etxt = event.text()
                    matched = False
                    if key == "½":
                        matched = (ek == 0x00BD or etxt == "½"
                                   or ek == int(Qt.Key.Key_onehalf))
                    elif key.startswith("Qt:"):
                        try: matched = (ek == int(key[3:]))
                        except Exception: pass
                    else:
                        ku = key.upper()
                        matched = (etxt.upper() == ku
                                   or (len(key) == 1 and ek == ord(ku.lower())))
                    if matched and not ctrl and not alt:
                        _toggle_pen(); return True
        return False


_key_filter   = _KeyFilter()
_pen_shortcut = [None]   # [QShortcut | None] for the current toggle key
...
class _SettingsDialog(QDialog):
    ...
    def _reset_to_default(self):
        ...
        defaults = {
            ...
            "erase_mode": "dot", "writing_mode": "balanced", "pen_type": "normal",
    ...
    def keyPressEvent(self, e):
        ...
        if e.key() in (Qt.Key.Key_Shift, Qt.Key.Key_Control,
        ...
        if txt.strip():
            ...
            # Non-printable key (F1-F12, etc.) — store as "Qt:<int>"
            self._set_key(f"Qt:{kint}")
...
def _show_welcome(force=False):
    ...
    def _open_settings_from_guide():
        dlg.accept()                              # close guide first
        QTimer.singleShot(0, _show_draw_settings) # then open settings
    open_s.clicked.connect(_open_settings_from_guide)
    ...
    # ── About (minimal, at bottom) ────────────────────────────────────────
    sep3 = QFrame(); sep3.setFrameShape(QFrame.Shape.HLine)
    lay.addWidget(sep3)
    about_lbl = QLabel(
        f"<b>{ADDON_NAME}</b>  v{ADDON_VERSION}  —  Created by Adel")
    about_lbl.setStyleSheet(f"font-size:10px;color:{t['dim']};margin-top:2px;")
...
def _show_draw_settings():
    ...
    if dlg.exec() == QDialog.DialogCode.Accepted:
        ...
        _save_cfg(cfg)
        print(f"[MDP] Settings saved — shortcut key: {cfg['pen_key']!r}")
        # Rebuild QShortcut immediately so new key works without restart
        _rebuild_shortcut()
        z = _controller._zone
...
def _install_shortcut():
    """
    Dual-path shortcut system:
    1. QShortcut on mw (PRIMARY) — fires even when the web view has focus
    2. QApplication event filter (SECONDARY) — handles mouse-button shortcuts
    """
    _rebuild_shortcut()
    app = QApplication.instance()
    if app:
        try: app.removeEventFilter(_key_filter)
        except Exception: pass
        app.installEventFilter(_key_filter)


def _rebuild_shortcut():
    """(Re)create the QShortcut for the current pen-toggle key."""
    # Delete previous shortcut
    old = _pen_shortcut[0]
    if old is not None:
        try: old.setEnabled(False); old.deleteLater()
        except Exception: pass
        _pen_shortcut[0] = None
        print("[MDP] Old QShortcut removed")

    key = _pen_key()
    print(f"[MDP] Rebuilding shortcut for key={key!r}")
    if not key or key.startswith("mouse_"):
        print("[MDP] Mouse-button key — using event filter only, no QShortcut")
        return

    # Build QKeySequence from the stored key string
    seq = _key_to_sequence(key)
    if seq is None or seq.isEmpty():
        print(f"[MDP] ERROR: could not build QKeySequence for key={key!r}")
        return
    print(f"[MDP] QKeySequence built: {seq.toString()!r}")

    try:
        if mw is None:
            print("[MDP] ERROR: mw is None — cannot attach QShortcut")
            return
        sc = QShortcut(seq, mw)
        sc.setContext(Qt.ShortcutContext.ApplicationShortcut)
        sc.activated.connect(_toggle_pen)
        sc.activatedAmbiguously.connect(_toggle_pen)
        _pen_shortcut[0] = sc
        print(f"[MDP] QShortcut attached to mw OK — {seq.toString()!r} → _toggle_pen")
        print(f"[MDP] Shortcut enabled: {sc.isEnabled()}, parent: {sc.parent()}")
    except Exception as e:
        print(f"[MDP] QShortcut error for key={key!r}: {e}")


def _key_to_sequence(key):
    """Convert a stored key string to a QKeySequence, or None on failure."""
    try:
        if key == "½":
            return QKeySequence(int(Qt.Key.Key_onehalf))
        if key.startswith("Qt:"):
            # Non-printable key stored as "Qt:<qt-key-int>"
            return QKeySequence(int(key[3:]))
        if len(key) == 1:
            # Single printable character — use its Qt key value directly
            # so we don't accidentally require Shift.
            # ord("p") == ord("P") == Qt.Key.Key_P so just use ord(lower)
            return QKeySequence(ord(key.lower()))
        # Fallback: try QKeySequence by name (works for "F7", "Space", etc.)
...
def _on_profile_loaded():
    ...
    def _restore_then_shortcut():
        _restore()
        # Install shortcut AFTER restore so mw widgets are fully ready
        QTimer.singleShot(100, _install_shortcut)

    QTimer.singleShot(500, _restore_then_shortcut)
